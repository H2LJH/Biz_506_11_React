...
/*
  ...
  rendering 되지 않기 때문에 static(정적) 페이지를 구성할때도 서버에 request를 보내고 서버가 반응하는 것에 따라
  전체 화면이 refresh 되는 전통적인 web 서버 구성보다 통신적인 비용발생이 매우 적다.
  실제 데이터에 따라 rendering되도록 설계된 react 이지만, 일반적인 html과 같은 화면을 구성할때도 react의 가상Dom, 부분렌더링 철학이 적용된다.

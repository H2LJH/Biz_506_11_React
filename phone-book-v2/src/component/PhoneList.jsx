...
({
  ...
  /*
    reducer 함수
    useReducer로 선언된 state 변수들을 통합하여 관리하는 함수이다.
  ...
  */
  ...
  {
    // 만약 action.type이 EDIT_FORM으로 설정되어서 reducer가 호출 되면 실행될 코드
    if(action.type === "EDIT_FORM")
    {
      return {name : action.name, number : action.number};
    }
    // input box에 문자열을 입력하는 change이벤트가 발생할때 defalut로 실행될 코드
